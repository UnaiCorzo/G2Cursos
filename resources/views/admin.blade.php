...
@section('user_content')
    ...
    <nav class="nav_admin" style="margin-top: 7.5%;">
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            <button class="nav-link active text-dark" id="nav-1-tab" data-bs-toggle="tab" data-bs-target="#nav-1"
            ...
            <button class="nav-link text-dark" id="nav-2-tab" data-bs-toggle="tab" data-bs-target="#nav-2" type="button"
            ...
            <button class="nav-link text-dark" id="nav-3-tab" data-bs-toggle="tab" data-bs-target="#nav-3" type="button"
                role="tab" aria-controls="nav-contact" aria-selected="false">{{ __('Usuarios') }}</button>
            <button class="nav-link text-dark" id="nav-4-tab" data-bs-toggle="tab" data-bs-target="#nav-4" type="button"
                role="tab" aria-controls="nav-contact" aria-selected="false">{{ __('Cursos') }}</button>
            <button class="nav-link text-dark" id="nav-5-tab" data-bs-toggle="tab" data-bs-target="#nav-5" type="button"
                role="tab" aria-controls="nav-contact" aria-selected="false">{{ __('Estadísticas') }}</button>
        </div>
    </nav>
                ...
                <div class="container mt-0">
                    <div class="table-responsive text-center">
                        <table class="table align-middle table-striped table-dark">
                            ...
                            @if (count($messages) == 0)
                                <tr>
                                    <td align="center" colspan="8">{{ __('No hay mensajes nuevos') }}</td>
                                </tr>
                            @endif
                            @foreach ($messages as $message)
                                <form class="form-control formulario_sesion" method="post"
                                    action="{{ route('contact_delete', [app()->getLocale(), $message->id]) }}"
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-1 px-3">
                                            <p class="m-0">{{ $message->comments }}</p>
                                        </td>

                                        <td class="p-1 px-3">
                                            <button type="submit" class="btn" name="admin_action" value="delete">
                                                <i class="btn bi bi-x-circle-fill" style="color: red; font-size: 30px"
                                                    type="submit" name="btn"></i>
                                            </button>
                                        </td>
                                    </tr>
                                </form>
                ...
                <div class="container mt-0">
                    <div class="table-responsive text-center">
                        <table class="table align-middle text-striped table-dark">
                            ...
                            @if (count($cvs) == 0)
                                <tr>
                                    <td align="center" colspan="8">{{ __('No hay peticiones nuevas') }}</td>
                                </tr>
                            @endif
                            @foreach ($cvs as $cv)
                                <form class="form-control formulario_sesion" method="post"
                                    action="{{ route('upgrade', app()->getLocale()) }}" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-1 pe-4">
                                            <button type="submit" class="btn" name="btn" value="accept">
                                                <i class="btn bi bi-check-circle-fill fa-lg" type="submit"
                    ...
                    <div class="table-responsive text-center">
                        <table class="table align-middle table-striped table-dark">
                            ...
                            @if (count($all_users) == 0)
                                <tr>
                                    <td align="center" colspan="8">{{ __('No hay usuarios') }}</td>
                                </tr>
                            @endif
                            @foreach ($all_users as $single_user)
                                <form class="form-control formulario_sesion" method="post"
                                    action="{{ route('profile_modify', [app()->getLocale(), $single_user->id]) }}"
                                    enctype="multipart/form-data">
                                    {{ csrf_field() }}
                    ...
                    <div class="table-responsive text-center">
                        <table class="table align-middle table-striped table-dark">
                            ...
                            @if (count($banned_users) == 0)
                                <tr>
                                    <td align="center" colspan="8">{{ __('No hay usuarios baneados') }}</td>
                                </tr>
                            @endif
                            @foreach ($banned_users as $banned_user)
                                <form class="form-control formulario_sesion" method="post"
                                    action="{{ route('profile_modify', [app()->getLocale(), $banned_user->id]) }}"
                                    enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <input type='hidden' name='panel' value="true">
                                    <tr>
                ...
                <div class="container mt-0">
                    <div class="table-responsive text-center">
                        <table class="table align-middle table-striped table-dark">
                            ...
                            @if (count($courses) == 0)
                                <tr>
                                    <td align="center" colspan="8">{{ __('No hay cursos creados') }}</td>
                                </tr>
                            @endif
                            @foreach ($courses as $course)
                                <form class="form-control formulario_sesion" method="post"
                                    action="{{ route('delete_course', [app()->getLocale(), $course->id]) }}"
                                    enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <tr>
                                        <td>
                        ...
                        <div class="row justify-content-center mx-4 mx-sm-0 mx-md-0 mx-lg-0">
                            <div class="col-lg-4 col-sm-6 mb-4">
                                <div class="portfolio-item">
                                    <a id="curso_{{ $best_course->id }}"
                                        href="{{ route('course', ['id' => $best_course->id, 'language' => app()->getLocale()]) }}"
                                        ...
                                        <div class="portfolio-caption">
                                            ...
                                            <div class="row m-0 p-0">
                                                <div class="col-6 p-0 docente_cursos text-start">
                                                    <p class="m-0 items">
                                                        {{ $best_course->teacher->name . ' ' . $best_course->teacher->surnames }}
                                                    </p>
                                                </div>
                                                <div
                                                    class="col-6 p-0 valoracion d-flex justify-content-end align-items-center">
                                                    <p class="m-0 me-1 pt-1 items">
                                                        {{ $best_course->ratings()->average('rating') }}
                                                    </p>
                                                    <div class="rating"
                                                        value="{{ round($best_course->ratings()->average('rating')) }}">
                                                    </div><br>
                                                    <p class="m-0 pt-1 items">({{ $best_course->ratings()->count() }})
                                                    </p>
                                                </div>
